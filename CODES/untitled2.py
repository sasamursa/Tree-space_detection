...
out = cv.VideoWriter("meanMethodeFiltered.avi",cv.VideoWriter_fourcc(*'DIVX'),5.0,(640,480))
...
while(1):
    ...
    bgrMean = bgr.mean()
    rgb_filtered = cv.inRange(bgr, 5+bgrMean, 255)
        ...
        for tree in sorted_trees:
            ...
            if len(sameLine[sameLine[:,1]>tree[1]])>1:
                ...
                if nextTree[1]-(tree[1]+tree[3]) > (meanOfHeihgt*1.5):
                    frame2 = cv.rectangle(frame2, (tree[0],(tree[1]+tree[3]+20)), ((nextTree[0]+nextTree[2]),nextTree[1]-20),(0,0,15),2)
                    cv.putText(frame2, 'Space', (tree[0],(tree[1]+tree[3]+40)), cv.FONT_HERSHEY_SIMPLEX,0.7, (0,0,15))
            if len(sameLine[sameLine[:,1] < tree[1]])==0:
                tree = tree.astype(np.int64)
                if tree[1]> (meanOfHeihgt*1.5):
                    frame2 = cv.rectangle(frame2, (tree[0],0), ((tree[0]+tree[2]),tree[1]-20),(0,0,15),2)
                    cv.putText(frame2, 'Space', (tree[0],20), cv.FONT_HERSHEY_SIMPLEX,0.7, (0,0,15))
    ...
    if k == 27:
         ...
         cv.imwrite('rapor/filtedPoints{}.png'.format(pcounter),result)
         pcounter +=1
